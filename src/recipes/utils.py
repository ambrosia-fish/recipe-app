...
def create_chart(chart_type, data, analysis_type):
    ...
    if analysis_type == 'ingredients':
        ...
        difficulty_counts = pd.Series([r.difficulty for r in data]).value_counts().sort_index()
        if chart_type == 'bar':
        ...
        if chart_type == 'bar':
            ...
            ranges = pd.cut(cooking_times, bins=5, labels=['0-60', '61-120', '121-180', '181-240', '241+'])
